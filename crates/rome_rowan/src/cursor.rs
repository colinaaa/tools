...
impl NodeData {
	...
	fn green_siblings(&self) -> Option<Siblings> {
		match &self.parent().map(|it| &it.green) {
			Some(Green::Node { ptr }) => {
				...
				let mut siblings = Siblings::new(parent);
				siblings.start_at_slot(self.slot());
				Some(siblings)
			}
			Some(Green::Token { .. }) => {
				debug_assert!(false);
	...
	fn next_sibling(&self) -> Option<SyntaxNode> {
		...
		siblings.forwards().find_map(|child| {
	...
	fn prev_sibling(&self) -> Option<SyntaxNode> {
		let siblings = self.green_siblings()?;
		siblings.backwards().find_map(|child| {
			child.element().into_node().and_then(|green| {
				let parent = self.parent_node()?;
				let offset = parent.offset() + child.rel_offset();
				Some(SyntaxNode::new_child(green, parent, child.slot(), offset))
	...
	fn next_sibling_or_token(&self) -> Option<SyntaxElement> {
		...
		siblings.forwards().next().and_then(|child| {
			let parent = self.parent_node()?;
			let offset = parent.offset() + child.rel_offset();
	...
	fn prev_sibling_or_token(&self) -> Option<SyntaxElement> {
		...
		siblings.backwards().next().and_then(|child| {
			let parent = self.parent_node()?;
			let offset = parent.offset() + child.rel_offset();
			Some(SyntaxElement::new(
				child.element(),
			...
			match node.parent() {
				Some(parent) => match parent.green() {
					NodeOrToken::Node(parent_green) => {
						new_green =
							parent_green.replace_slot(node.slot() as usize, Some(new_green.into()));
						node = parent;
					}
					_ => unreachable!(),
				},
...
impl<'a> Siblings<'a> {
	pub fn new(parent: &'a GreenNodeData) -> Self {
		Self {
			parent,
			start_slot: 0,
		}
	...
	pub fn start_at_slot(&mut self, start: u32) {
		assert!(
			(start as usize) < self.parent.slots().len(),
			"Start slot out of bounds"
		);
		self.start_slot = start;
	}

	pub fn forwards(&self) -> Children<'a> {
	...
	pub fn backwards(&self) -> impl Iterator<Item = Child<'a>> {
		let mut slots = self.parent.slots().enumerate();
