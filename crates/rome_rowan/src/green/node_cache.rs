...
fn element_id(elem: Option<GreenElementRef<'_>>) -> Option<*const ()> {
	Some(match elem? {
	...
	pub(crate) fn node(
		...
		let entry = self.nodes.raw_entry_mut().from_hash(hash, |node| {
			node.0.kind() == kind && node.0.slots().len() == slots_ref.len() && {
				...
				let lhs = lhs.map(|slot| element_id(slot.as_ref()));

				let rhs = slots_ref.iter().map(|(_, element)| {
					// Converts &Option<NodeOrToken<GreenNode, GreenToken> to Option<NodeOrToken<&GreenNode, &GreenToken>>
					let element_ref = element.as_ref().map(|e| e.as_deref());
